  ...
  return (
    <div className="rounded-lg border border-gray-300 bg-white p-5">
      <div className="flex items-start justify-between">
        ...
        <Icon className={`h-5 w-5 ${alerta ? 'text-red-600' : 'text-gray-400'}`} />
      </div>
      <p className={`mt-3 text-3xl font-bold ${alerta ? 'text-red-600' : 'text-black'}`}>
        {valor}
      </p>
    </div>
  )
  ...
  return (
    <div className="rounded-lg border border-gray-300 bg-white">
      <div className="flex items-center gap-2 border-b border-gray-200 px-5 py-4">
        ...
        <h3 className="text-base font-semibold text-black">Servicios de Hoy</h3>
      </div>

      {servicios.length === 0 ? (
        <p className="px-5 py-6 text-sm text-gray-500">No hay servicios agendados para hoy.</p>
      ) : (
...
function AlertasYEvaluacionesPanel({ alertas, evaluaciones }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {alertas.length === 0 ? (
        <div className="rounded-lg border border-gray-300 bg-white p-5">
          <p className="text-sm text-gray-500">No hay insumos en alerta. Todo en orden.</p>
        </div>
      ) : (
        alertas.map((insumo) => (
      ...
      <div className="rounded-lg border border-gray-300 bg-white">
        <div className="border-b border-gray-200 px-5 py-4">
          <h3 className="text-base font-semibold text-black">Últimas Evaluaciones</h3>
        </div>
        {evaluaciones.length === 0 ? (
          <p className="px-5 py-6 text-sm text-gray-500">Aún no hay evaluaciones registradas.</p>
        ) : (
          <ul className="divide-y divide-gray-200">
            {evaluaciones.slice(-3).reverse().map((ev) => (
              <li key={ev.id_evaluacion} className="px-5 py-4">
